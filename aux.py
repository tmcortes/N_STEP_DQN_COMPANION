...
def plot_each_epoch(i_epoch, phase,save_path,minimum_time, total_results,total_loss_epoch,total_reward_epoch,
    ...
    plt.subplot2grid((1,3), (0,0))
    plt.plot(total_G_energy_epoch, 'b:')
    plt.legend(["Energy discounted reward"])
    plt.xlabel("Epoch")

    plt.title(phase+" | Energy discounted reward")
    
    plt.subplot2grid((1,3), (0,1))
    plt.plot(total_G_time_epoch, 'b:')
    plt.legend(["Time discounted reward"])
    plt.xlabel("Epoch")

    plt.title(phase+" | Time discounted reward")
    
    
    plt.subplot2grid((1,3), (0,2))
    plt.plot(total_G_total_epoch, 'b-.')
    plt.xlabel("Epoch")
    plt.legend(["Total discounted reward"])
    plt.title("Total discounted reward (G)")

    fig1.savefig(save_path+'/'+phase+'_DISCOUNTED_REWARD.jpg')
    plt.close(fig1)
    
    plt.close('all')
    
    
def plot_each_epoch_together(i_epoch,save_path,minimum_time, total_results_train,total_loss_epoch_train,total_reward_epoch_train,
                             maximum_time,total_time_execution_epoch_train,total_reward_energy_epoch_train,
                             total_reward_time_epoch_train,ex_rate,total_results,total_loss_epoch_val,total_reward_epoch_val,
                             total_time_execution_epoch_val,total_reward_energy_epoch_val,total_reward_time_epoch_val,
    ...
    plt.ylabel("Epsilon")
    plt.plot(ex_rate, 'c')
    fig1.savefig(save_path+'/04_LOSSES_EXPLORATION.jpg')
    ...
    plt.subplot2grid((1,3), (0,0))
    plt.plot(total_G_energy_epoch_train, 'b:', label='train')
    plt.plot(total_G_energy_epoch_val,'m:',label='val')
    plt.xlabel("Epoch")
    plt.title("Energy discounted reward")
    plt.legend()
    
    plt.subplot2grid((1,3), (0,1))
    plt.plot(total_G_time_epoch_train,'b:',label='train')
    plt.plot(total_G_time_epoch_val,'m:', label='val')
    plt.xlabel("Epoch")
    plt.title("Time discounted reward")
    plt.legend()
    
    plt.subplot2grid((1,3), (0,2))    
    plt.plot(total_G_total_epoch_train,'b-.',label='train')
    plt.plot(total_G_total_epoch_val,'m-.',label='val')
    plt.xlabel("Epoch")
    plt.title("Total discounted reward (G)")
    plt.legend()
    
    fig1.savefig(save_path+'/03_DISCOUNTED_REWARDS.jpg')
    plt.close(fig1)
    plt.close('all')
