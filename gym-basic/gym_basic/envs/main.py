...
class BasicEnv(gym.Env):
    ...
    def __init__(self, display=False, test=False):
        ...
        if cfg.TEMPORAL_CONTEXT:
            if ONLY_RECOGNITION == True:
                if Z_hidden_state:
                    self.observation_space = gym.spaces.Discrete(N_ATOMIC_ACTIONS+VWM+N_OBJECTS+TEMPORAL_CTX+Z_HIDDEN) # Next Action + Action Recog + VWM + Obj in table + Z
                else:
                    self.observation_space = gym.spaces.Discrete(N_ATOMIC_ACTIONS+VWM+N_OBJECTS+TEMPORAL_CTX)
            else:
                if Z_hidden_state:
                    self.observation_space = gym.spaces.Discrete(N_ATOMIC_ACTIONS*2+VWM+N_OBJECTS+TEMPORAL_CTX+Z_HIDDEN) # Next Action + Action Recog + VWM + Obj in table + Z
                else:
                    self.observation_space = gym.spaces.Discrete(N_ATOMIC_ACTIONS*2+VWM+N_OBJECTS+TEMPORAL_CTX)
        ...
        # 2) Robot execution times
        if self.test:
            if cfg.TEMPORAL_CONTEXT:
                
                self.robot_execution_times = np.load("./robot_execution_times",allow_pickle=True)
            else:
    ...
    def reset(self):
        ...
        if cfg.TEMPORAL_CONTEXT:
            ...
            if ONLY_RECOGNITION==True:
                if Z_hidden_state:
                    self.state = np.concatenate((ac_rec,vwm,oit,self.human_time_to_finish,self.robot_execution_times ,z))
                else:
                    self.state = np.concatenate((ac_rec,vwm,oit,self.human_time_to_finish,self.robot_execution_times ))
            else:
                if Z_hidden_state:
                    self.state = np.concatenate((ac_pred,ac_rec,vwm,oit,self.human_time_to_finish,self.robot_execution_times ,z))
                else:
                    self.state = np.concatenate((ac_pred,ac_rec,vwm,oit,self.human_time_to_finish,self.robot_execution_times ))
        else:
            if ONLY_RECOGNITION==True:
                if Z_hidden_state:
    ...
    def transition(self):
        ...
        if cfg.TEMPORAL_CONTEXT:
            ...
            # 1) El output de María ya está normaliado entre -1,1
            self.human_time_to_finish = np.array([np.load(path_time_to_finish_pkl,allow_pickle=True).numpy().squeeze()[frame_to_read]])
            
            # 2) Las estimaciones del robot las normalizamos
            if np.std(self.robot_execution_times) == 0:
                robot_execution_times_normalized = self.robot_execution_times
            else:
                robot_execution_times_normalized = self.robot_execution_times - np.mean(self.robot_execution_times) / np.std(self.robot_execution_times)
            # pdb.set_trace()
            if ONLY_RECOGNITION==True:
                if Z_hidden_state:
                    self.state = np.concatenate((ac_rec,vwm,oit,self.human_time_to_finish,robot_execution_times_normalized,z))
                else:
                    self.state = np.concatenate((ac_rec,vwm,oit,self.human_time_to_finish,robot_execution_times_normalized))
            else:
                if Z_hidden_state:
                    self.state = np.concatenate((ac_pred,ac_rec,vwm,oit,self.human_time_to_finish,robot_execution_times_normalized,z))
                else:
                    self.state = np.concatenate((ac_pred,ac_rec,vwm,oit,self.human_time_to_finish,robot_execution_times_normalized))
        else:
            if ONLY_RECOGNITION==True:
                if Z_hidden_state:
                    self.state = np.concatenate((ac_rec,vwm,oit,z))
