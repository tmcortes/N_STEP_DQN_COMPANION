...
class BasicEnv(gym.Env):
    ...
    def __init__(self, display=False, test=False):
        ...
        self.human_time_to_finish = np.load(path_time_to_finish_pkl, allow_pickle=True)
        
        # 2) Robot execution times
        if self.test:
            path_robot_execution_times_pkl = os.path.join(videos_realData[video_idx], "robot_execution_times") # Path to pickle as ./root_realData/videos_realData[video_idx]/frame_pkl
            self.robot_execution_times = np.load(path_robot_execution_times_pkl,allow_pickle=True)
        else:
